...
function SpeechTranslatorPage() {
  ...
  const translateSpeechText = async (finalChunk) => {
    if (!finalChunk.trim()) {
      toast.error("Please Speak anything to translate");
      return;
    }
    setIsTranslating(true);
    try {
      ...
      setTranslatedText(
        (prev) =>
          `${prev}${!prev.endsWith(" ") ? " " : ""}${res.data.translatedText} `,
      );
      setDetectedLang(res.data.detectedLanguageName);
      toast.success("Translated! ✨");
    } catch (err) {
  ...
  const initSpeechRecognition = useCallback(() => {
    ...
    recognition.onresult = async (event) => {
      ...
      if (final) {
        await translateSpeechText(final);
        setSourceText((prev) => prev.trim() + " " + final.trim() + " ");
      }
    };
          ...
          <div>
            ...
            >
              <span className="gradient-text">
                Real-Time Auto Speech Translator
              </span>
            </h1>
            <p
              style={{
                      ...
                      {sourceText && (
                        <>
                          <button
                            ...
                            title={`${isPlayingText ? "Stop" : "Read aloud"}`}
                          >
                            <i
                              className={`bi ${isPlayingText ? "bi-volume-mute" : "bi-volume-up"}`}
                            />
                          </button>
                          <button
            ...
            >
              ...
              >
                {audioWaveform ? (
                  ...
                  >
                    {isTranslating ? "⚡ Processing..." : "🎤 Ready to record"}
                  </div>
                )}
              </div>
...
export default SpeechTranslatorPage;
